...
export function createBlizzard(env = process.env, fetcher = fetch) {
  ...
  async function request(path, family = 'profile') {
    ...
    if (!r.ok) {
      ...
      throw new ApiError(r.status === 404 ? 'Character or guild is unavailable in Blizzard’s API.' : r.status === 429 ? 'Blizzard is busy. Please try again later.' : 'Blizzard data is temporarily unavailable.', r.status === 404 ? 404 : 502);
    }
    return r.json();
  }
  ...
  return {
    ...
    async character(record) {
      ...
      const results = await Promise.allSettled([request(`${base}/equipment`), request(`${base}/statistics`), profile.active_spec?.id ? request(`/data/wow/playable-specialization/${profile.active_spec.id}`, 'static') : Promise.resolve(null)]);
      const [equipment, statistics, spec] = results.map(r => r.status === 'fulfilled' ? r.value : null);
